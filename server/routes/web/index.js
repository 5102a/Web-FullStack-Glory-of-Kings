module.exports = app => {
  ...
  router.get('/news/list', async (req, res) => {
    ...
    for (let i = 0; i < 3;) {
      ...
      }).populate('categories').limit(2).lean()

      let p = true
      for (let i = 0; i < hot.newsList.length; i++) {
        if (hot.newsList[i].title == temp[0].title) {
          p = false
          break
        }
  ...
  router.get('/heroes/list', async (req, res) => {
    ...
    for (let i = 0; i < 1000;i++) {
      ...
      for (let n = 0; n < 3;n++) {
        p = true
        p=hot.heroList.every((item,i,arr)=>{
          if(item.name==temp[n].name){
            return false
          }
          return true
        })
        ...
        if(p){
          hot.heroList.push(temp[n])
        }
        if(hot.heroList.length>=10){
          break
        }
      }
